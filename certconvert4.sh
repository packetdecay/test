...
usage() {
  ...
  echo ""
  echo "Examples:"
  echo "  $0 -i cert.pfx -o cert             # Outputs cert.pem, cert.key, cert.crt, cert-ca.pem, cert-fullchain.pem"
...
case "${input_ext,,}" in
  ...
  pfx|p12)
    if [[ "$is_basename_output" == true ]]; then
      ...
      for f in "$key" "$pem" "$ca" "$fullchain"; do check_overwrite "$f" || exit 3; done
